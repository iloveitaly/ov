...
func (root *Root) searchFilter(ctx context.Context, forward bool, lineNum int, searcher Searcher) {
	if searcher == nil {
	...
	eg.Go(func() error {
		n := lineNum
		for i := 0; i < 100; i++ {
			nextN, err := root.Doc.searchLine(ctx, searcher, forward, n)
			if err != nil {
				log.Println("searchFilter", err)
				break
			}
			str := root.Doc.LineString(nextN)
			log.Printf("searchFilter %d:%s", n, str)
			n = nextN + 1
		}
		root.sendSearchQuit()
		return nil
